...
import PredictionDetail from '@/components/tokenPredictor/PredictionDetail';
import TokenDetailAnalysis from '@/components/tokenPredictor/TokenDetailAnalysis';
...
const TokenPredictor = () => {
  ...
  const openPredictionDetail = (prediction: any) => {
  ...
  return (
    <DashboardLayout
      ...
      description="AI-powered price predictions and sentiment analysis for cryptocurrencies."
    >
        ...
        <TabsContent value="predictions" className="space-y-6">
          ...
          <div className="space-y-4">
            {predictions.map((prediction) => (
              <Card key={prediction.id} className="p-4 bg-feature-gradient border-white/5">
                <div className="flex items-start">
                  <div className="mr-4 mt-0.5">
                    <div className={`w-10 h-10 rounded-full ${
                      prediction.status === 'accurate' ? 'bg-green-500/20' : 'bg-orange-500/20'
                  ...
                  <div className="flex-1">
                    <div className="flex items-center mb-1">
                      <h3 className="font-semibold text-white">{prediction.token} {prediction.timeframe} Prediction</h3>
                      <Badge className={`ml-2 ${
                        prediction.status === 'accurate' ? 'bg-green-500/20 text-green-400' : 'bg-orange-500/20 text-orange-400'
                      }`}>
                        {prediction.status === 'accurate' ? 'Accurate' : 'Partially Accurate'}
                      </Badge>
                      <span className="text-white/50 text-xs ml-auto">{prediction.date}</span>
                    </div>
                    
                    <p className="text-white/70 text-sm mb-2">
                      Predicted price: ${prediction.predictedPrice.toLocaleString()} • 
                      Actual price: ${prediction.actualPrice.toLocaleString()} • 
                      Difference: {prediction.difference >= 0 ? '+' : ''}{prediction.difference}%
                    </p>
                    
                    <div className="flex justify-between items-center text-sm">
                      <span className="text-white/50">AI confidence: <span className="text-white">{prediction.confidence}</span></span>
      ...
      <PredictionDetail 
        open={showPredictionDetail}
        onClose={() => setShowPredictionDetail(false)}
        prediction={selectedPrediction}
      />

      {/* Token Detail Analysis Modal */}
      <TokenDetailAnalysis
